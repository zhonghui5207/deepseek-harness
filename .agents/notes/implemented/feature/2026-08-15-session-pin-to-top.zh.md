...
用户需要会话粘性置顶：被置顶的聊天会话离开其所在工作区分组（以及单列表），留在「工作区／会话」区头上方的**置顶**栏，直到用户取消置顶。最近更新、Last updated 提升和 Host 手动会话顺序都不得打乱该位置。这不是活动置顶（已删除的 `touchSession` 链），也不是归档：会话保持可见。
...
**置顶顺序是 workspace domain 全局单例（`workspaceDomainState.pinnedSessionIds`）上的新字段，后置顶的在前，覆盖在 workspace 记账之上；显示把这些 id 抽到 client `tree.ts` 派生的一条侧栏置顶区，落在既有视图顺序之后；wire 面沿用归档已有的全快照姿态。**
...
- UI：会话行菜单增加「置顶会话」／「取消置顶」；置顶行带一枚小图钉。`derivePinned` 按 Host 顺序收集可见置顶 id，供「工作区／会话」区头上方的**置顶**栏使用。`deriveGroups` 与单列表展示从各分组／单列表中去掉这些 id，避免同一会话出现两次。栏在空时隐藏，并在浏览器视图 store 里记住折叠。单列表记账仍保存未分区的视图顺序；取消置顶后行回到记账中的位置。置顶行不可拖拽；未置顶拖拽写入未分区记账。搜索排序不变。
...
**在每个 workspace 分组／单列表内部分区。** 否决：产品要求是工作区上方一条侧栏置顶区，而不是钉在每个项目内部。
...
置顶顺序由 Host 持久，并在标签页与刷新之间共享。尚无置顶重排 UI；要改相对顺序只能取消置顶再置顶（后置顶前置）。置顶行不可拖拽；未置顶拖拽仍写入原来的 Host 或浏览器本地记账。搜索结果不做置顶分区。`workspace.list` 增加 `pinnedSessionIds` 是 pre-release 直改。

## 测试

领域测试覆盖前置、已置顶空操作、取消置顶、归档剥离置顶、未知 id 拒绝、跨重启恢复与旧介质默认。Host 测试覆盖 RPC、置顶时的 `session-not-found`、list 基线与 changed 帧。运行时测试覆盖一元回声、帧安装与 in-flight 基线屏蔽。UI 测试覆盖菜单置顶／取消置顶、图钉、侧栏置顶栏、从 workspace 分组和单列表中抽走，以及单列表置顶 → 拖未置顶行 → 取消置顶后回到记账位置。workspace-management e2e 钉住装配后的全链路（置顶 → 持久顺序 → reload 后仍置顶 → 取消置顶）。

...
> Division of labor: this document = the layering model + the channel-independent RPC protocol; the protocol's Web implementation combines HTTP uplink with the [WebSocket downlink carrier](2026-08-04-websocket-downlink-carrier.md), the browser object layer is in the [web client architecture note](2026-07-19-gui-web-client-architecture.md), and the shipped Electron host and distribution decision is in the [Desktop shell note](2026-08-14-electron-desktop-shell-and-distribution.md).
...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) boots the Web profile in-process and reuses the same web client packages through an ephemeral loopback HTTP/WebSocket carrier; an IPC carrier remains a future transport option.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser and sandboxed Desktop-renderer access; zero workspace dependencies (the registry arrives by structural injection); Desktop binds loopback on an ephemeral port |
...
| Application | `@deepseek-ai/dsh` (apps/cli) + `dsh-web-frontend` (apps/web) + `@deepseek-ai/dsh-desktop` (apps/desktop) | Coarse bin dispatch, the thin Vite main, and the Electron process/window lifecycle | Applications own launch, presentation, and exit semantics; reusable capabilities remain packages |
...
1. **Pick a fetch impersonation**: browser or Desktop same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. future Electron IPC, see the "Subclass table" below).
...
The shipped applications preserve the division: `dsh web` and Desktop mount Host, the Web carrier, and the browser composition (Desktop inside a sandboxed window), while `dsh --profile headless` mounts a direct core runner with zero Host, HTTP, or ports. ACP-class protocol bridges do not follow the client-carrier checklist: they expose core to the external ecosystem and mount directly via `ctx.plugin(entry-point plugin)` without fetch.
...
| IPC bridge subclass (hypothetical example — Desktop does not use it today) | a future Electron carrier | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |

...
**The executable manifest owns its full workspace peer closure and installed application assets.** Desktop depends on `@deepseek-ai/dsh` as the single installed owner of `config/agent-presets`; its launcher overlays that directory as the system root and validates `code`, `cordis`, `minimal`, and `standard` before opening a window. Shared launcher overlays live in [`dsh-app-boot`](../../../../packages/boot/app-boot/README.md): Desktop and `dsh` both apply `homePatchPath`, `shippedAgentPresetOverlay`, and `resolveTelemetryPatch`, so any non-empty `DSH_TELEMETRY_DISABLED` disables the telemetry row before Loader mount. `verify-runtime-closure` checks the Desktop manifest as well as the Python runtime carrier. The release workflow then starts the actual unpacked executable with fresh Harness and Electron user-data directories and creates a session from every system preset, catching missing assets and packages that a shell-only smoke cannot without colliding with an installed running instance. On Linux, the workflow restores root ownership and mode `4755` on Electron's unpacked `chrome-sandbox` helper before that smoke, matching the installed helper instead of disabling Chromium sandboxing. `asar` is disabled because profile fallback symlinks and Loader resolution require real package directories; this choice is explicit until those mechanisms gain an archive-safe installation representation.
...
- Preview releases include the black-whale application icon but remain unsigned and unnotarized, provide [bounded release discovery](../feature/2026-08-14-desktop-release-update-discovery.md) without an unattended updater, and build one native architecture per release runner.

...
describe('web e2e: workspace management (create / rename / flat view / hover affordances)', () => {
  ...
  it('pins the seeded session from its row menu and keeps the order across reload', async () => {
    ...
    await expect.poll(() => page.getByRole('group', { name: 'Pinned' }).count(), { timeout: 10_000 }).toBe(1)
    const pinnedRow = page.getByRole('group', { name: 'Pinned' }).locator('[role="treeitem"]').nth(1)

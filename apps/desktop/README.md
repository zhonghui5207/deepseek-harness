...
Desktop uses the same Harness home as the CLI: `$DSH_HOME`, otherwise `~/.dsh`. Profiles, `settings.yaml`, `.credentials.yaml`, sessions, and model routes therefore carry across without a Desktop-specific migration. Any non-empty `DSH_TELEMETRY_DISABLED` disables the shared telemetry row at Desktop boot, matching `dsh`. Closing the last window exits on Windows and Linux; macOS keeps the application active and recreates the window on activation. A single-instance lock focuses the existing window, and every quit path gives the Cordis tree a bounded graceful shutdown before Electron exits.

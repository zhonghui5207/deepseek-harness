 ...
 * except workspace Rename/Delete and session Rename/Fork/Pin/Unpin/Archive; the session
 ...
 * @param props.onPin - pin a session by id.
 * @param props.onUnpin - unpin a session by id.
...
export function SessionNodeItem({
  ...
  /** Pin this session (row menu action). */
  onPin: (id: SessionNode['id']) => void
  /** Unpin this session (row menu action). */
  ...
  const sessionMenuItems = [
    ...
    node.pinned === true
      ? { id: 'unpin', label: t('menu.unpinSession'), icon: <IconPinOutline16 /> }
  ...
  const ownRow = (
    ...
    >
      ...
      {node.pinned === true && (
        <span className={css.pin} aria-hidden="true"><IconPinOutline16 size={12} /></span>
      )}
      {/* A blank New Session row is a provisional placeholder: nothing has
          happened in it yet, so a "now" timestamp and the row verbs
          (rename/fork/pin/archive) would all act on content that does not

...
供应用启动器（[`dsh`](../../../apps/cli/README.md)、[`DSH Desktop`](../../../apps/desktop/README.md) 与 [`dsh-acp-demo`](../../examples/acp-demo/README.md)）共用的启动粘合层：每个启动器都是这些辅助函数之上的精简自执行组合，并以自身诊断前缀参数化。这样，Loader 故障与进程关闭行为只由一处负责，不会在已发布产物之间逐渐分化。
...
| `createProcessShutdown(dispose, forceExit?, complete?, timeoutMs?)` | 围绕一个有上限的应用 disposer 合并普通完成请求；中断会先 dispose 再退出，重复中断或超时则强制立即退出 |
| `PROCESS_SHUTDOWN_TIMEOUT_MS` | 长驻启动器在绕过卡死 disposer 前共用的默认五秒宽限期 |
...
| `resolveProfileDir` / `initProfile` / `loadProfile` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `indexComposedRows` / `healProfilesModuleFallback` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` / `PROFILE_ROOT_FILENAME` / `homePatchPath` / `shippedAgentPresetOverlay` / `TELEMETRY_ROW_ID` / `resolveTelemetryPatch` | Profile 机制与共享启动器 overlay（见 [Profile](#profiles)） |
...
配置中的裸插件 specifier（`@deepseek-ai/dsh-*`、npm 包）通常通过 Cordis Loader 的内部模块 loader 解析。默认情况下，它们从配置目录解析；封闭运行时会向 `boot` 或 `mountRootInclude` 传入 `bareModuleBaseUrl`，使已安装包树保持权威，即使配置位于另一个 Node 项目中也不受遮蔽。相对 specifier 始终以配置目录为基准解析。当 Electron 等嵌入方不暴露 Node 内部 ESM loader 时，模块基准会安装一个基于公开 `createRequire` 与动态 import 的适配器：每个子树先从自身 parent 解析，再回退到已安装宿主；相对解析失败仍保持为相对失败。该适配器可以导入模块，但有意不提供模块缓存 HMR。仓库 Node bin 仍安装 Loader 的可选对等依赖 `node-addon-require-builtin`。构建后的 `dsh-app-boot` 产物内嵌静态挂载的 Include 实现，但仍将 Loader 保持为外部依赖，因此 include 树与宿主会绑定到同一个 Loader peer。`pnpm dsh` 源码路径还会将 manifest 声明的 workspace 包映射到其 TypeScript 源码；其配置门禁要求每个随附的原始／Web 裸插件都出现在解析所用 manifest 的 `dependencies` 中。

此包不包含 loader 钩子，也不提供开发模式接口。[`dsh` 应用](../../../apps/cli/README.md) 持有自己的 Node 源码启动钩子，并在启动序列中使用这些 helper；构建后的消费方仍使用普通 Node 包解析。

## Profiles

profile 是位于 `$DSH_HOME/profiles/<name>` 下的目录（harness home 由 [`resolveDshHome`](../../util/home-paths/README.md) 解析：先取 `$DSH_HOME`，否则取 `~/.dsh`），其中包含一个 `package.json`（树外插件 `dependencies`，加上 profile manifest `dsh.profile` 及其有序的 `bundles` 层列表）和用户自己的 `cordis.patch.yml`。组合包是在 manifest 中声明 `"dsh": { "bundle": { "patch": "./cordis.patch.yml" } }` 的 npm 包；`loadProfile` 以双锚点解析每个 `dsh.profile.bundles` 名称（先从 dsh 安装目录，再从 profile 目录），列出的包若没有组合包声明则明确报错。`composeEntries` 通过 include 自己的 `applyEntryPatches` 在空条目列表之上应用各 patch 层，因此组合、标志推导和配置 dump 绝不会与实际启动内容发生偏离。`indexComposedRows` 按字符串 id 索引这些组合后的配置项，供启动器做行检查。`homePatchPath` 每次调用都解析 `$DSH_HOME/cordis.patch.yml`。当组合结果包含 `agent-presets` 行时，`shippedAgentPresetOverlay` 会改写其系统根，启动器自行提供安装目录。当组合结果包含 `TELEMETRY_ROW_ID`（`session-telemetry-otel`）时，`resolveTelemetryPatch` 会把任何非空的 `DSH_TELEMETRY_DISABLED` 转成禁用 overlay。`dsh` 与 Desktop 都会应用这些 overlay；ACP demo 不启动 profile。`healProfilesModuleFallback` 维护扁平的 `$DSH_HOME/profiles/node_modules` 目录（安装 manifest 的依赖与必需对等依赖闭包中的每个包对应一个符号链接），使任意 profile 中的裸插件名都能经 Node 常规的逐级向上查找解析，而无需由 pnpm 管理随安装内置的包。`PROFILE_TEMPLATES`（`web`、`headless`）在首次使用时自动初始化；其他名称在 `initProfile` 创建之前都会明确报错（即 `dsh plugin` 路径）。`loadProfile` 会将与安装自有组合包元组完全一致的列表规范化为随发行版交付的模板，同时保留 manifest 中其他所有字段；一旦条目有任何额外、缺失或重排，该列表就归用户所有并保持不变。
...
需要实时编辑 profile 的启动器会在启动后调用 `watchUserPatches`，并通过有上限的关闭流程 dispose 返回的 watcher。即使该文件或其直接父目录不存在，监视器仍会监视确切路径；它会串行处理突发变更，并按调用方的层次顺序重新组合用户 patch（组合包层在下、overlay 在上）。读取失败、解析失败或 Loader 候选被拒时，最后一个可用树会继续运行；HMR 服务记录错误后广播 `hmr/config-update-failed(filename, Error)`，并隔离观察方的失败。上下文 dispose 时会关闭 watcher，并等待进行中的刷新结束。`dsh` 启用这条路径；Desktop 当前只在启动时加载 patch 层。
...
- **公开模块适配器不提供模块 HMR**：没有 Loader Node 内部 ESM 集成的嵌入方可以解析并导入锚定包，但不能使对应模块缓存失效；启动器启用 `watchUserPatches` 后，配置 patch 替换仍可工作。
